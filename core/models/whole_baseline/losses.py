...
def similarity_mask_old(feat_all, bsz, simil, batch_size, thres_ratio=0.3, temp_tSNE=0.1):
    ...
    if simil == 'tSNE':
        simil_thres = simil_mean + thres_ratio * (simil_max - simil_mean)
        # print(simil_thres)
        contrast_mask[simil_mat_avg > simil_thres] = 1
    elif simil == 'kNN':
        contrast_mask[:batch_size, :batch_size][
            simil_mat_avg[:batch_size, :batch_size] <
            torch.kthvalue(simil_mat_avg[:batch_size, :batch_size],
                           int(simil_thres), 1, True)[0]] = 1
        contrast_mask[:batch_size, :batch_size][
            simil_mat_avg[:batch_size, :batch_size] <
            torch.kthvalue(simil_mat_avg[:batch_size, :batch_size],
                           int(simil_thres), 0, True)[0]] = 1

    #pos_pairs.update(contrast_mask.sum().item() / batch_size, bsz)
